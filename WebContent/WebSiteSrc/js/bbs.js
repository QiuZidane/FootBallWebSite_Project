...
EventUtil.addLoadHandler(function() {
	...
	function fnSend() {		
		...
		if (reg.test(oUserName.value)) {
			...
			oLi.innerHTML = "<div class=\"userPic\"><img src=\"" + bbsData.imgsrc + "\"></div>\
							 <div class=\"content\">\
							 	<div class=\"userName\"><a href=\"javascript:;\">" + bbsData.username + "</a>:</div>\
								<div class=\"msgInfo\">" + bbsData.msginfo + "</div>\
								<div class=\"times\"><span>" + bbsData.smtimes + "</span><a class=\"\" href=\"javascript:;\"></a></div>\
							 </div>"; // qiusd注释  </span><a class=\"del\" href=\"javascript:;\"> 改了，不允许删除
...
function uploadData() {
	$.ajax({
		...
		type: "POST",
		//提交的网址
		url: clubserver.URL + "UpLoadBBSData",
		//提交的数据
		data: {
			imgsrc: bbsData.imgsrc,
			username: bbsData.username,
			msginfo: bbsData.msginfo,
			smtimes: bbsData.smtimes
		},
		//返回数据的格式
	...
	$.ajax({
		...
		complete: function(XMLHttpRequest, textStatus) {
			...
			try {
				...
				$('#bbslist').empty();
				//重新生成结点数据
				var parentNode = document.getElementById('bbslist');
...
//					console.log(jsonObject[key]);
					...
					oLi.innerHTML = "<div class=\"userPic\"><img src=\"" + data["imgsrc"] + "\"></div>\
							 <div class=\"content\">\
								...
								<div class=\"msgInfo\">" + data["msginfo"] + "</div>\
								<div class=\"times\"><span>" + data["smtimes"] + "</span><a class=\"\" href=\"javascript:;\"></a></div>\
							 </div>";
					oFragement.appendChild(oLi);
				}
				var announce = document.createElement("li");
				announce.innerHTML = "<div class=\"userPic\"><img src=\"img/face.gif\" /></div>\
							<div class=\"content\"><div class=\"userName\"><a href=\"javascript:;\"></a>管理员:</div>\
